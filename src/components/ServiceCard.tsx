  ...
  return (
    ...
    >
      ...
      {/* Hover overlay with details */}
            ...
            >
              {description && (
                <p className="text-[11px] md:text-sm text-foreground/90 line-clamp-4 leading-relaxed text-center px-2 mb-3">
                  {description.replace(/<[^>]*>/g, '').slice(0, 100)}{description.replace(/<[^>]*>/g, '').length > 100 ? '...' : ''}
                </p>
              )}
              <button className="bg-accent hover:bg-accent/90 text-accent-foreground font-bebas uppercase tracking-widest text-sm md:text-base px-6 py-1.5 rounded-lg transition-all hover:scale-105 shadow-lg">
                View Details
              </button>
            </motion.div>
          </div>
